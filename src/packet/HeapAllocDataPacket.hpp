...
/************************************************************************/
/*
/* 与CDataPacket相同，但内存管理使用HeapAlloc/HeapFree进行
/*
/************************************************************************/
#include <stdlib.h>
...
namespace alloctrace
{
	namespace packet
	{
		...
		class CHeapAllocDataPacket : public CDataPacketReader
		{
		public:
			/* 写入二进制数据
		...
		private:
			...
			* 检查引用赋值
			*/
			inline CHeapAllocDataPacket& operator = (const CHeapAllocDataPacket& an);
			/*{
			//禁止引用赋值！
			Assert(false);
			}*/
		public:
			/* 写入原子数据的运算符重载 */
			template <typename T>
			inline CHeapAllocDataPacket& operator << (T val)
			{
			...
			inline CHeapAllocDataPacket& operator << (const char* val)
			{
				writeString(val, strlen(val));
				return *this;
			}
			inline CHeapAllocDataPacket& operator << (char* val)
			{
				writeString(val, strlen(val));
				return *this;
			}
			inline CHeapAllocDataPacket& operator << (const wchar_t* val)
			{
				writeWideString(val, wcslen(val));
				return *this;
			}
			inline CHeapAllocDataPacket& operator << (wchar_t* val)
			{
				writeWideString(val, wcslen(val));
				return *this;
			}
		...
		private:
			CHeapAllocDataPacket(const CHeapAllocDataPacket &another);
			/*{
		...
		public:
			...
			CHeapAllocDataPacket(LPCVOID lpDataPtr, size_t dwSize)
		...
		protected:
			...
			void setSize(size_t dwNewSize)
			{
				...
				//内存长度按CAllocTraceDataPacket::MemoryAlginmentSize字节对齐
				//数据长度增加4字节，用于增加0终止，以便进行字符串的结尾调试
				dwNewSize = (dwNewSize + MemoryAlginmentSize) & (~(MemoryAlginmentSize - 1));
#ifdef _DPPACK_MEM_CHK_
				char *pMemory = (char*)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, dwNewSize + sizeof(INT));
#else
				char *pMemory = (char*)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, dwNewSize);
#endif
				...
				*(PINT)m_pMemoryEnd = 0xCCDDCCEE;
				...
				{
					_Assert();
					HeapFree(GetProcessHeap(), 0, pOldMemory);
				}
			}
			/* 写入ANSI短字符串以及字符串数据
		...
		private:
			static const size_t MemoryAlginmentSize = 1024 * 1024;	//数据包内存长度对齐边界
		};
