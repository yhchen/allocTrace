...
#define FONTNAME		"ËÎÌו"//"Courier New"
#define FONTSIZE		12
...
typedef HANDLE (CALLBACK* LPREMOVEPROP)(HWND,const char*);  // VC2010 issue
...
static struct tag_TreeListSession
{
    ...
    TREELIST_EDIT_CB                 *pCBValidateEdit;
};
...
#else
static LPREMOVEPROP pRemoveProp = RemovePropA;
#endif
...
static void *TreeList_Internal_DictGetPtr(HWND hWndParent,HWND hWndAny)
{
    ...
    pTreeListDict = (TreeListDict*)GetPropA(hParent,TREELIST_PROP_VAL); // Extract the dict pointer
    if(!pTreeListDict)
...
static BOOL TreeList_Internal_DictUpdate(BOOL Clear,TreeListSession *pSession,HWND hWndParent,HWND hWndAny)
{
    ...
    pTreeListDict = (TreeListDict*)GetPropA(hParent,TREELIST_PROP_VAL); // Extract the dict pointer from the parent
    if(Clear == FALSE)
    {
        if(!pTreeListDict)
        {
            ...
            if(SetPropA(hParent,TREELIST_PROP_VAL,pTreeListDict) == FALSE)
                return FALSE;
...
static void TreeList_Internal_DestroyEditBox(TreeListSession *pSession)
{
    ...
    if(pSession->HwndEditBox)
    {
		...
		SetWindowLongPtr(pSession->HwndEditBox, GWLP_WNDPROC, (ULONG_PTR)pSession->ProcEdit);
	...
	case WM_PAINT:
		{
			...
			if(hDC)
			{
				...
				hBitMapMem = CreateCompatibleBitmap(hDC, rect.right - rect.left, rect.bottom - rect.top);
				hOldBitMap = (HBITMAP)SelectObject(hDCMem, hBitMapMem);

				CallWindowProc (pSession->ProcTreeList, hWnd, Msg, (WPARAM)hDCMem, lParam);
...
static LRESULT TreeList_Internal_HandleTreeMessages (HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    ...
    TVITEM              Treeitem            = {0};
    WNDPROC             ProcParent          = (WNDPROC)GetPropA(hWnd,"WNDPROC");
    ...
    TreeListNode        *pNode              = 0;
    LPWINDOWPOS         lpPos               = 0;
	char				ColumnText[MAX_PATH];
	TreeListNodeData	*pColumnData		= 0;
    LRESULT				lResult				= 0;
    ...
    case WM_PAINT:
        {
            ...
            if(hDC)
            {
                ...
                hBitMapMem= CreateCompatibleBitmap(hDC, pSession->RectTree.right - pSession->RectTree.left, 
					pSession->RectTree.bottom - pSession->RectTree.top);
    ...
    case TREELIST_WM_EDIT_NODE:
        {
            ...
            if(pNode)
            {
				...
				pColumnData = pNode->pNodeData[pSession->EditedColumn];
                // a Numeric only text box
                if(pColumnData->Numeric == TRUE)
                    dwStyle = WS_CHILD|WS_VISIBLE|ES_NUMBER;
                else
                    dwStyle = WS_CHILD|WS_VISIBLE;

				ColumnText[0] = 0;
                pColumnData->TextCB(pSession, pSession->EditedColumn, pColumnData->pExternalPtr, ColumnText, MAX_PATH - 1);

                pSession->HwndEditBox = CreateWindowExA(0,
					"EDIT",
                    ColumnText,
                    dwStyle,
                ...
                if(pSession->HwndEditBox )
                {
					...
					pSession->ProcEdit = (WNDPROC)SetWindowLongPtr(pSession->HwndEditBox, GWLP_WNDPROC, (ULONG_PTR)TreeList_Internal_HandleEditBoxMessages); // Subclassing the control (it will help trapping the enter key press event on a dialog)	
					SendMessageA(pSession->HwndEditBox, WM_SETFONT, (WPARAM)pSession->FontHandleEdit, (LPARAM)TRUE);
                    SetFocus(pSession->HwndEditBox);
					SendMessageA(pSession->HwndEditBox, EM_SETSEL, strlen(ColumnText), strlen(ColumnText)); // Select all text
    ...
    case TREELIST_WM_EDIT_ENTER:
        {
            ...
            if(GetWindowTextA(pSession->HwndEditBox,pSession->EditBoxBuffer,TREELIST_MAX_STRING) > 0)// Get edit box text
			{
				pColumnData = pNode->pNodeData[pSession->EditedColumn];
				ColumnText[0] = 0;
				pColumnData->TextCB(pSession, pSession->EditedColumn, pColumnData->pExternalPtr, ColumnText, MAX_PATH - 1);

                if(strcmp(pSession->EditBoxBuffer,ColumnText) != 0)
                {
                    // Call the user!
                    if(pSession->pCBValidateEdit)
                    {
                        ...
                        if(pSession->pCBValidateEdit((NODE_HANDLE)pSession,pNode->pNodeData[pSession->EditedColumn]->pExternalPtr,pSession->EditBoxBuffer, pSession->EditBoxOverrideBuffer) == TRUE)
                        {
                            ...
                            pColumnData->Altered = TRUE; // Set "changed" flag so we will be able to color it later
                            // Update CRC
                            pColumnData->CRC = TreeList_Internal_CRCCreate(pColumnData,
                                (sizeof(TreeListNodeData) - sizeof(pColumnData->CRC))); // Set the srs sig
                        }
                    ...
                    case CDDS_ITEMPOSTPAINT:
                        {
                            ...
                            do
							{
								...
								pSession->ColumnsCount = Header_GetItemCount(pSession->HwndHeader);
								if(pSession->ColumnsCount == -1)
									break; // No columns info, nothing to do
								...
								pColumnData = pNode->pNodeData[0];
								ColumnText[0] = 0;
								pColumnData->TextCB(pSession, 0, pColumnData->pExternalPtr, ColumnText, MAX_PATH - 1);
								DrawTextA(hDC,ColumnText,(int)strlen(ColumnText),&RectText,DT_NOPREFIX | DT_CALCRECT);
								...
								if(Header_GetItemRect(pSession->HwndHeader,0,&RectHeaderItem) == FALSE)
									break;// Error getting the rect
                            
                            
								RectText.right = RectHeaderItem.right; // Set the right side
								TreeList_Internal_DeflateRectEx(&RectText,2, 1); // Defalate it
								DrawTextA(hDC,ColumnText,(int)strlen(ColumnText), &RectText, DT_NOPREFIX | DT_END_ELLIPSIS); // Draw it
								...
								for (iCol =1; iCol < pSession->ColumnsCount; iCol++)
								{
									...
									if(pNode->pNodeData[iCol])
									{
										...
										if(pNode->pNodeData[iCol])
										{
											...
											pColumnData = pNode->pNodeData[iCol];
											ColumnText[0] = 0;
											pColumnData->TextCB(pSession, iCol, pColumnData->pExternalPtr, ColumnText, MAX_PATH - 1);
											DrawTextA(hDC,ColumnText,(int)strlen(ColumnText), &RectText, DT_NOPREFIX | DT_END_ELLIPSIS);
										}
								...
								if((pSession->CreateFlags & TREELIST_DRAW_EDGE) == TREELIST_DRAW_EDGE)
								{
									hDCTemp = GetDC(pSession->HwndParent);
									DrawEdge(hDCTemp,&pSession->RectBorder, EDGE_ETCHED,BF_RECT);
									ReleaseDC(pSession->HwndParent, hDCTemp);
								}
							}
							while (FALSE);
            ...
            case NM_CLICK:
                {
                    ...
                    if(TreeTestInfo.hItem)
                    {
                        ...
                        if(pSession->HwndEditBox)
							SendMessageA(pSession->HwndParent, TREELIST_WM_EDIT_ENTER, 0, 0);
                        else
                        {
                            if(iCol > 0)
...
TREELIST_HANDLE TreeListCreate(HINSTANCE Instance, HWND Hwnd,RECT *pRect,DWORD dwFlags, TREELIST_EDIT_CB *pFunc)
{
    ...
    if(!hDllHandle)
    {
        hDllHandle = LoadLibraryA("user32.dll");
        if (hDllHandle != NULL)
        {
    ...
    do
    {
        ...
        pSession->FontHandleTreeList = CreateFontA(FONTSIZE, 0, 0, 0, 500, FALSE, FALSE, FALSE,
            DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
            CLIP_CHARACTER_PRECIS, PROOF_QUALITY,
            FF_DONTCARE, FONTNAME);
        
        // Create a font for the Header control
        pSession->FontHandleHeader = CreateFontA(FONTSIZE, 0, 0, 0, 700, FALSE, FALSE, FALSE, // 700 = Bold font
            DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
            CLIP_CHARACTER_PRECIS, PROOF_QUALITY,
            FF_DONTCARE, FONTNAME);
        
        // Create a font for the edit box
        pSession->FontHandleEdit = CreateFontA(FONTSIZE, 0, 0, 0, 500, FALSE, FALSE, FALSE,
            DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
            CLIP_CHARACTER_PRECIS, PROOF_QUALITY,
            FF_DONTCARE, FONTNAME);
        ...
        pSession->HwndTreeView = CreateWindowEx(0,                                  // Extended styles
            ...
            0,                                                                      // Control caption
            WS_CHILD | WS_VISIBLE | TVS_FULLROWSELECT | TVS_NOHSCROLL | TVS_NOTOOLTIPS | TVS_HASBUTTONS | TVS_HASLINES | TVS_LINESATROOT | TVS_SHOWSELALWAYS,
            0,0,0,0,                                                                // Position
            pSession->HwndParent,                                                   // Parent window handle
            0,                                                                      // Control's ID
            pSession->InstanceParent,                                               // Instance
		...
		SendMessageA(pSession->HwndTreeView, WM_SETFONT, (WPARAM)pSession->FontHandleTreeList, (LPARAM)TRUE);
        pSession->ProcTreeList = (WNDPROC)SetWindowLongPtr(pSession->HwndTreeView, GWLP_WNDPROC, (ULONG_PTR)TreeList_Internal_HandleTreeMessagesEx); // Sub classing the control

        // Sub class the parent window
        if(PrevInstance == FALSE)
        {
			pSession->ProcParent = (WNDPROC)SetWindowLongPtrA(pSession->HwndParent, GWLP_WNDPROC, (ULONG_PTR)TreeList_Internal_HandleTreeMessages); // Sub classing the control
			SetPropA(pSession->HwndParent, "WNDPROC", pSession->ProcParent);
        }
		...
		SendMessageA(pSession->HwndHeader, WM_SETFONT, (WPARAM)pSession->FontHandleHeader, (LPARAM)TRUE);
...
int  TreeListDestroy         (TREELIST_HANDLE ListTreeHandle)
{
    ...
    if(pSession->HwndTreeView)
    {
		...
		(WNDPROC)SetWindowLongPtrA(pSession->HwndParent, GWLP_WNDPROC, (ULONG_PTR)pSession->ProcParent); // Restore the original wnd proc to the parent
...
////////////////////////////////////////////////////////////////////////////////////
//
// Function:    TreeListPaintTo
// Description: Paint to DC
// Parameters:  TREELIST_HANDLE a valid handle to a listtree control
//				HDC					target device context
// Return:      
//
////////////////////////////////////////////////////////////////////////////////////
VOID TreeListPaintTo(TREELIST_HANDLE ListTreeHandle,HDC hDC)
{
	TreeListSession *pSession = (TreeListSession*)ListTreeHandle;
	...
	if(pSession)
	{
		...
		SendMessageA(pSession->HwndHeader, WM_ERASEBKGND, (WPARAM)hDC, TRUE);
		SendMessageA(pSession->HwndHeader, WM_PAINT, (WPARAM)hDC, PRF_CLIENT | PRF_NONCLIENT);


		SendMessageA(pSession->HwndTreeView, WM_ERASEBKGND, (WPARAM)hDC, TRUE);
		SendMessageA(pSession->HwndTreeView, WM_PAINT, (WPARAM)hDC, PRF_CLIENT | PRF_NONCLIENT);
...
////////////////////////////////////////////////////////////////////////////////////
//
// Function:    TreeListGetTreeHwnd
// Description: Get treeview HWND
// Parameters:  TREELIST_HANDLE a valid handle to a listtree control
// Return:      
//
////////////////////////////////////////////////////////////////////////////////////
HWND TreeListGetTreeHwnd(TREELIST_HANDLE ListTreeHandle)
{

	TreeListSession *pSession = (TreeListSession*)ListTreeHandle;

	if(pSession)
		return pSession->HwndTreeView;
	return NULL;
